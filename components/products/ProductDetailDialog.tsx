...
import { ChevronLeft, BarChart3, Trophy, MessageSquare, Star, ExternalLink } from 'lucide-react'
...
function ReviewSection({ reviews, summary, loading }: {
    ...
    return (
      <div className="bg-gray-50 rounded-lg p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-gray-500 text-sm font-medium">レビュー</span>
        </div>
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {summary && (
            <div className="flex items-center gap-1.5 ml-2">
              ...
              <span className="text-sm font-medium text-gray-700">{summary.avg_rating.toFixed(1)}</span>
              <span className="text-xs text-gray-400">({summary.total_reviews}件)</span>
            </div>
          )}
        </div>
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto">
        {filtered.map((r, i) => (
          <div key={`${r.order_number}-${i}`} className="bg-white rounded-lg p-3 border border-gray-100">
            ...
            {r.title && (
              <div className="text-sm font-medium text-gray-700 mb-0.5">{r.title}</div>
            )}
            {r.review_body && (
              <div className="text-xs text-gray-500 leading-relaxed line-clamp-3">{r.review_body}</div>
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose() }}>
      <DialogContent className="max-w-[95vw] lg:max-w-5xl xl:max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        {/* Reviews (product view only) */}
        {!isSkuView && (
          <ReviewSection reviews={reviews} summary={reviewSummary} loading={reviewsLoading} />
        )}
